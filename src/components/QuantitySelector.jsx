...
function QuantitySelector({ onAddToCart }) {
    ...
    return (
        <div>
            <div className='flex flex-row text-red-500 justify-between w-[100px] mt-4'>
                <button className="border rounded-l-lg p-2 text-center text-3xl" onClick={() => changeQuantity(-1)}>-</button>
                <input className="border p-2 text-center w-[50px] font-medium" type="text" value={quantity} readOnly />
                <button className="border rounded-r-lg p-2 text-center text-3xl" onClick={() => changeQuantity(1)}>+</button>
            </div>
